...
let cos x n =
    let term x i =                             
        (((-1.0) ** (float i)) * (x ** (float (cos_pow i)))) /  (fact(cos_pow i))
    [0..(n/2)] |> List.map (term x) |> List.reduce (+)
...
let sin x n =
    ...
    then 0.0
        ...
        let term x i =                             
            (((-1.0) ** (float i)) * (x ** (float (sin_pow i)))) /  (fact (sin_pow i))
        [0..((n-1)/2)] |> List.map (term x) |> List.reduce (+)
